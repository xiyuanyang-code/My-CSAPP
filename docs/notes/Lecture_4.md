...
- 原精度计算对 GPU 的显存要求极高
    - 因为在训练阶段反向传播的过程中计算图的存储和计算需要占据额外的存储空间（相比于后续固定参数的推理阶段），因此为了保证显存的高效利用，提出了**混合精度训练**，核心目标是在保持相同性能的前提下进行混合精度训练。
    - 直接使用 FP16 等精度更低的数字会导致严重的下溢出和数值计算的不稳定现象。

混合精度训练：使用FP16计算，使用FP32做模型权重更新
- 模型权重使用FP32存储
- 将模型权重从FP32转换为FP16
- 使用FP16进行前向计算，得到损失值（loss）
- *缩放损失值*
- 使用FP16进行反向计算，得到FP16的模型梯度
- 将模型梯度从FP16转换为FP32
- *FP32 模型梯度除以前面的缩放值*
- 更新FP32的模型权重
...
- **减少精度误差累计带来的影响**：从权重中读取和更新权重都是 FP32 精度，因此梯度下降做权重更新的时候误差影响较小
    - 因为梯度的绝对值的数量级可能远小于权重本身的数量级
- 缓解梯度下溢出的问题: Underflow Problems
    - **损失值缩放**的方法
    - 增加 exp 域的位宽，使用 BF16 进行存储

> IEEE-754 标准下的浮点数 ($M$ 位指数, $N$ 位尾数)，可以近似连续的表示 $[2^{-2^{M-1} + 2 - N}, (2 - 2^{-N}) \times 2^{2^{M-1} - 1}]$ 范围内的浮点数值。

- M 增大, N 减小: 表示范围变大，但是两个数之间的 gap 会变大（尤其是大数之间），精度更低
- M 减小，N 增大：表示范围变小，但是精度更高
