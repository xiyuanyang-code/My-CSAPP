...
- `40131b <string_length>`: 输入字符串并读取字符串的长度，字符串的长度必须等于 6: `cmp    eax,0x6`
- 接下来一共是六次循环
    - `movzx  ecx,BYTE PTR [rbx+rax*1]`: 读取第 rax 个字符，存储 ecx 寄存器中
    - `and    edx,0xf`: 和 15 进行 and 操作（相当于只取后四位）
    - `movzx edx, BYTE PTR [rdx+0x4024b0]`: 以刚才计算出的低 4 位作为索引（Index），去地址 0x4024b0 处的数组里找一个字符。
